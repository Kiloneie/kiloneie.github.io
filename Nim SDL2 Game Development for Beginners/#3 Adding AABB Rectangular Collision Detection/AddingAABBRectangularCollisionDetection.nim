...
#AABB collision detection of rect1 vs rect2
...
#Line 1 checks if obj1 left side, is to the left of obj2 right side
  #Line 2 checks if obj1 right side, is to the right of obj2 left side
  #Line 3 checks if obj top side, is above obj2 bottom side
  #Line 4 checks if obj1 bottom side, is below obj2 top side
  #If all of the 4 lines are "true", collision detected!
...
proc collisionAABB(obj1: Player, obj2: seq[Wall]): bool = 
  let wall = obj2[0].image
  for w in obj2: #Checks every single wall at all times
    if (obj1.x.float64 < w.x + wall.w.float64 and
    obj1.x + obj1.image.w.float64 > w.x and
    obj1.y < w.y + wall.h.float64 and
    obj1.y + obj1.image.h.float64 > w.y):
      echo "Collision detected!"
      return true
...
#Line 1 checks if obj1 left side, is to the left of obj2 right side
#Line 2 checks if obj1 right side, is to the right of obj2 left side
#Line 3 checks if obj1 top side, is above obj2 bottom side
#Line 4 checks if obj1 bottom side, is below obj2 top side
  #If all of the 4 lines are "true", collision detected!
...
if init(app):
  ...
  while not done: #if done = false -> we had an error -> terminate
    ...
    var rect1 = sdl.Rect(x: 55, y: 55, w: 50, h: 50)
    var rect2 = sdl.Rect(x: 170, y: 60, w: 10, h: 10)

    #Rectangle 1 - Red
    discard app.renderer.setRenderDrawColor(0xFF, 0x00, 0x00, 0xFF)
    discard app.renderer.renderDrawRect(addr(rect1))

    #Rectangle 2 - blue
    discard app.renderer.setRenderDrawColor(0x00, 0x00, 0xFF, 0xFF)
    discard app.renderer.renderDrawRect(addr(rect2))
    ...
    if kbd[ScancodeUp]    > 0: 
      if collisionAABB(player, walls) == false:
        player.y -= player.speed * delta 
    if kbd[ScancodeDown]  > 0: 
      if collisionAABB(player, walls) == false:
        player.y += player.speed * delta 
    if kbd[ScancodeLeft]  > 0: 
      if collisionAABB(player, walls) == false:
        player.x -= player.speed * delta 
    if kbd[ScancodeRight] > 0: 
      if collisionAABB(player, walls) == false:
        player.x += player.speed * delta
