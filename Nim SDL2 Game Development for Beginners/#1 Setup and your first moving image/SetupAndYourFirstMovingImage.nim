...
if init(app):
  #Here we load our image, and give it parameters
  var
    image = newImage() #new empty image
    imagePos: tuple[x, y: float64]
    imageSpd = 200.0 #pixels per second

  #Load the image, if not turn off the program
  if not image.load(app.renderer, getAppDir() / "img/img1a.png"):
    sdl.logCritical(sdl.LogCategoryError,
                    "Can't load image: %s",
                    img.getError())
    done = true #No error -> proceed

  #Placing our image in the center of the window
  imagePos.x = ScreenW div 2 #or ScreenW / 2
  ...
  while not done: #if done = false -> we had an error -> terminate
    ...
    if kbd[ScancodeUp]    > 0: imagePos.y -= imageSpd * delta 
    if kbd[ScancodeDown]  > 0: imagePos.y += imageSpd * delta 
    if kbd[ScancodeLeft]  > 0: imagePos.x -= imageSpd * delta 
    if kbd[ScancodeRight] > 0: imagePos.x += imageSpd * delta
